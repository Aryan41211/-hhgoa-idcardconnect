...
export type SquadMember = { name?: string; photoBase64?: string; crop?: { x: number; y: number; width: number; height: number } | null };
export type Squad = {
  id: string;
...
export async function createSquad(team?: string): Promise<Squad> {
  const id = crypto.randomBytes(5).toString("hex");
  const squad: Squad = { id, members: new Array(MAX_SLOTS).fill(undefined), team, createdAt: new Date().toISOString() };
...
export async function addMember(id: string, member: SquadMember): Promise<Squad | null> {
  ...
  squad.members[idx] = { name: member.name, photoBase64: member.photoBase64, crop: member.crop };
  await fs.mkdir(DIR, { recursive: true });
  await fs.writeFile(file(id), JSON.stringify(squad));
  return squad;
}
